...
internal void DrawEditorUI()
{
    ...
    if (core->is_in_editor)
    {
        ...
        if (TsUIDropdown("Options..."))
        {
            ...
            if (TsUIToggler("Draw Colliders", core->draw_colliders))
                ...
                if (is_index_mode)
                {
                    ...
                    for (int i = 1; i < core->world_data->entity_count; i++)
                    {
                        ...
                        if (entity->entity_id > 0)
                        {
                            ...
                            if (TsUIToggler(entity->name, (core->selected_entity ? core->selected_entity->entity_id == i : 0)))
                            {
                                core->selected_entity = entity;
                            }
                            else
                            {
                                if (core->selected_entity && core->selected_entity->entity_id == i)
                                {
                                    core->selected_entity = 0;
                                }
                            }
                    ...
                    for (int i = 0; i < GENERALISED_ENTITY_TYPE_MAX; i++)
                    {
                        if (TsUICollapsable(GetGeneralisedEntityTypeName(i)))
                        {
                            for (int j = 1; j < core->world_data->entity_count; j++) // TEMP: Need to sort these before-hand. Will eventually get too inefficient.
                            {
                                ...
                                {
                                    if (TsUIToggler(entity->name, (core->selected_entity ? core->selected_entity->entity_id == j : 0)))
                                    {
                                        core->selected_entity = entity;
                                    }
                                    else
