...
internal void RenderSprites()
{
	...
	{
		SpriteComponent *sprite_component = &core->run_data->entity_components.sprite_components[i];
		if (!sprite_component->parent_entity_id)
			continue;
		PositionComponent *pos_comp = GetPositionComponentFromEntityID(sprite_component->parent_entity_id);
		Entity *entity = GetEntityWithID(sprite_component->parent_entity_id);
		
		if (sprite_component->sprite_data.dynamic_sprite)
		{
			DynamicSpriteData *dynamic_sprite = &global_dynamic_sprite_data[sprite_component->sprite_data.dynamic_sprite];
			
			AnimationComponent *animation_comp = GetAnimationComponentFromEntityID(sprite_component->parent_entity_id);
			Assert(animation_comp);
			
			v2 render_size = v2zoom(v2(dynamic_sprite->source.z * sprite_component->sprite_data.scale.x * (sprite_component->is_flipped ? -1.0f : 1.0f), dynamic_sprite->source.w * sprite_component->sprite_data.scale.y));
			
			// NOTE(randy): Determine proper position of the Sprite - Offset is defaulted to BottomCentre
			v2 render_pos = v2view(pos_comp->position);
			render_pos = V2AddV2(render_pos, v2(render_size.x / -2.0f, -render_size.y));
			render_pos = V2AddV2(render_pos, v2zoom(v2((dynamic_sprite->offset.x + sprite_component->sprite_data.offset.x) * (sprite_component->is_flipped ? -1.0f : 1.0f), dynamic_sprite->offset.y + sprite_component->sprite_data.offset.y)));
			...
			ArcPushTexture(dynamic_sprite->texture_atlas,
						   ...
						   (core->run_data->editor_state && entity == core->run_data->entity_editor.selected_entity ? V4MultiplyV4(v4(1.0f, 0.8f, 0.8f, 1.0f), sprite_component->sprite_data.tint) : sprite_component->sprite_data.tint),
						   sprite_component->sprite_data.render_layer);
		}
		else if (sprite_component->sprite_data.static_sprite)
		{
			StaticSpriteData *static_sprite = &global_static_sprite_data[sprite_component->sprite_data.static_sprite];
			
			v2 render_size = v2zoom(v2(static_sprite->source.z * sprite_component->sprite_data.scale.x * (sprite_component->is_flipped ? -1.0f : 1.0f), static_sprite->source.w * sprite_component->sprite_data.scale.y));
			
			// NOTE(randy): Determine proper position of the Sprite - Offset is defaulted to BottomCentre
			v2 render_pos = v2view(pos_comp->position);
			render_pos = V2AddV2(render_pos, v2(render_size.x / -2.0f, -render_size.y));
			render_pos = V2AddV2(render_pos, v2zoom(v2((static_sprite->offset.x + sprite_component->sprite_data.offset.x) * (sprite_component->is_flipped ? -1.0f : 1.0f), static_sprite->offset.y + sprite_component->sprite_data.offset.y)));
