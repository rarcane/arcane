internal void
PreLoadTs2dTexture(char *name, Ts2dTexture *texture)
{
	Log("[Assets] Preloading texture \"%s\" into %p.", name, texture);
}

internal void
PreLoadTs2dFont(char *name, Ts2dFont *font)
{
	Log("[Assets] Preloading font \"%s\" into %p.", name, font);
}

internal void
LoadTs2dTexture(char *name, Ts2dTextureLoadInfo *load_info)
{
	void *png_data = 0;
	u32 png_data_len = 0;
	char png_path[256] = {0};
...
internal void
LoadTs2dFont(char *name, Ts2dFontLoadInfo *load_info)
{
	void *png_data = 0;
	u32 png_data_len = 0;
...
internal void
PostLoadTs2dTexture(Ts2dTextureLoadInfo *texture_load_info, Ts2dTexture *texture)
{
	Log("[Assets] Postloading texture %p.", texture);
	texture->width = texture_load_info->width;
	texture->height = texture_load_info->height;
	*texture = Ts2dTextureInit(TS2D_TEXTURE_FORMAT_R8G8B8A8, texture->width,
							   texture->height, texture_load_info->texture_data);
	free(texture_load_info->texture_data);
}

internal void
PostLoadTs2dFont(Ts2dFontLoadInfo *load_info, Ts2dFont *font)
{
	Log("[Assets] Postloading font %p.", font);
	*font = Ts2dFontInit(TS2D_TEXTURE_FORMAT_R8G8B8A8,
...
internal void
CleanUpTs2dTexture(Ts2dTexture *texture)
{
	if (texture->id)
	{
		Ts2dTextureCleanUp(texture);
	}
}

internal void
CleanUpTs2dFont(Ts2dFont *font)
{
	if (font->texture.id)
	{
		CleanUpTs2dTexture(&font->texture);
	}
}
