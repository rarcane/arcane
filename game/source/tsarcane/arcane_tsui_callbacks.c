internal void
ArcaneUIRenderWidget(i32 style_flags, TsUIWidget *widget, v4 clip, void *user_data,
    ...
    if (style_flags & ARCANE_UI_STYLE_MENU)
    {
        ...
        switch (widget->type)
        {
            ...
            case TSUI_WIDGET_toggler:
            {
                ...
                {
                    widget->toggler.toggle_transition += (1 - widget->toggler.toggle_transition) * 8.f * core->delta_t;
                }
                else
                {
                    widget->toggler.toggle_transition += (0 - widget->toggler.toggle_transition) * 8.f * core->delta_t;
                }
                break;
            }
            ...
            case TSUI_WIDGET_int_slider:
            {
                ...
                if (widget->type == TSUI_WIDGET_slider)
                {
                    ...
                    slider_text = MakeCStringOnMemoryArena(core->frame_arena, "%s (%.2f)", widget_text, widget->slider.value);
                }
                else if (widget->type == TSUI_WIDGET_int_slider)
                {
                    ...
                    slider_text = MakeCStringOnMemoryArena(core->frame_arena, "%s (%i)", widget_text, widget->slider.value);
                }
                
                Ts2dPushFilledRect(v4(0.2f, 0.2f, 0.2f, 0.2f),
                                   v4(widget_x, widget_y + 6, widget_w, widget_h - 13));
                
                Ts2dPushFilledRect(v4(fill_brightness, fill_brightness, fill_brightness, 0.6f),
                                   v4(widget_x, widget_y + 6, widget_w * fill_percentage,
                                      widget_h - 13));
                
                Ts2dPushRect(v4(border_brightness, border_brightness, border_brightness, 1),
                             v4(widget_x, widget_y + 6, widget_w, widget_h - 13));
            ...
            default:
            TsUIRenderWidgetDefaultTs2d(style_flags, widget, clip, 0, widget_is_hot, widget_is_active);
            break;
        }
    }
...
internal void
ArcaneUIRenderWindow(i32 style_flags, TsUIWindow *window, void *user_data)
{
    TsUIRenderWindowDefaultTs2d(style_flags, window, 0);
}