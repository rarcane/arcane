...
void
_TsAssetsLoadDirectoryItems(char *path, int *item_count_ptr, char ***items_ptr)
{
    int item_count = 0;
    int item_capacity = 0;
    char **items = 0;
...
LogWarning("[Assets] Directory search task space exhausted.");\
    ...
    for(DirectorySearchTask *task = first_search_task; task; task = task->next)
    {
        // NOTE(rjf): Load file list
        {
            ...
            snprintf(search_pattern, MAX_PATH, "%s\\%s\\%s*", global_tsassets->assets_root_path, path, task->sub_search_pattern);
...
void
_TsAssetsInitWatchDirectories(void)
{
    for(int i = 0; i < global_tsassets->asset_type_count; ++i)
    {
        ...
        snprintf(path, sizeof(path), "%s\\%s\\", global_tsassets->assets_root_path, watch_directory->path);
...
void
_TsAssetsCheckAssetDirectories(void)
{
    for(int i = 0; i < global_tsassets->asset_type_count; ++i)
    {
        TsAssetsWatchDirectory *watch_directory = global_tsassets->asset_directories + i;
