...
#if TS2D
f32
TsUIGetTextWidthDefaultTs2d(i32 style_flags, f32 text_scale, char *text, void *user_data)
{
...
f32
TsUIGetTextHeightDefaultTs2d(i32 style_flags, f32 text_scale, char *text, void *user_data)
{
    Ts2d *renderer = user_data;
    Ts2dFont *font = Ts2dGetDefaultFont();
...
void
TsUIRenderWidgetDefaultTs2d(i32 style_flags, TsUIWidget *widget, v4 clip, void *user_data, b32 widget_is_hot, b32 widget_is_active)
{
...
void
TsUIRenderWindowDefaultTs2d(i32 style_flags, TsUIWindow *window, void *user_data)
{
    Ts2d *renderer = user_data;
...
void
TsUIUpdateAndRenderWidget(i32 style_flags, TsUIWidget *widget, v4 clip)
{
...
void
TsUIUpdateAndRenderWindow(TsUIWindow *window)
{
...
void
TsUIWidgetInit(TsUIWidget *widget, TsUIWidgetType type, TsUIID id, i32 style_flags,
               v4 rect, char *text, TsUIWindow *parent_window)
{
    ...
    if(global_tsui->input_grid_group_active && !TsUIIDEqual(id, TsUIIDNonInteractable()))
    {
        ...
        if(widget_is_in_active_input_group)
        {
            ...
            {
                case TSUI_DIRECTION_LEFT:  { global_tsui->current_state.input_grid_state.position.x -= 1; break; }
                case TSUI_DIRECTION_RIGHT: { global_tsui->current_state.input_grid_state.position.x += 1; break; }
                case TSUI_DIRECTION_UP:    { global_tsui->current_state.input_grid_state.position.y -= 1; break; }
                case TSUI_DIRECTION_DOWN:  { global_tsui->current_state.input_grid_state.position.y += 1; break; }
                default: break;
            }
...
void
TsUIBeginFrame(TsUIFrameData *frame)
{
    ...
    // NOTE(rjf): Update windows (if not stale, make stale. If stale, delete).
    {
        ...
        for(i32 i = 0; i < (i32)global_tsui->window_count; ++i)
        {
            ...
            else
            {
                ...
                if(window->active && (V4HasPoint(window->rect, v2(global_tsui->cursor_x, global_tsui->cursor_y)) ||
                                      global_tsui->dragging_window == window) && global_tsui->allowed_window == 0)
                {
...
void
TsUISetFrameCallbacks(TsUICallbacks *callbacks)
{
    if(callbacks)
    {
...
TsUIEndFrame(void)
{
    ...
    v4 default_clip =
    {
        0, 0, global_tsui->render_width, global_tsui->render_height,
    };
...
void
TsUIBeginInputGroup(void)
{
    global_tsui->input_grid_group_active = 1;
}

void
TsUIEndInputGroup(void)
{
    global_tsui->input_grid_group_active = 0;
    global_tsui->input_grid_selected_group_active = 0;
}
...
void
_TsUIPushWidth(f32 width, b32 calculate_with_text)
{
...
void
_TsUIPushHeight(f32 height, b32 calculate_with_text)
{
...
void
TsUIPushWidth(f32 width)
{
    _TsUIPushWidth(width, 0);
}

void
TsUIPushHeight(f32 height)
{
    _TsUIPushHeight(height, 0);
}

void
TsUIPushAutoWidth(void)
{
    _TsUIPushWidth(0, 1);
}

void
TsUIPushAutoHeight(void)
{
    _TsUIPushHeight(0, 1);
}

void
TsUIPushPosition(v2 pos)
{
    TsUIPushX(pos.x);
    TsUIPushY(pos.y);
}
...
void
TsUIPushColumn(v2 position, v2 size)
{
...
void
TsUIPushCenteredColumn(v2 size, u32 number_of_widgets)
{
    v2 position =
    {
...
void
TsUIPushRow(v2 position, v2 size)
{
    TsUIPushPosition(position);
    TsUIPushSize(size);
...
void
TsUIPushAutoRow(v2 position, f32 height)
{
    TsUIPushPosition(position);
    TsUIPushAutoWidth();
...
void
TsUIPushInputGridDirection(i32 direction)
{
...
void
TsUIPopInputGridDirection(void)
{
...
void
TsUIUpdateAndGetCurrentState(i32 style_flags, char *text, v4 *out_rect,
...
void
TsUIWidgetUpdateNonInteractable(v4 rect)
{
...
b32
_TsUIUpdateClickableWidget(TsUIID id, i32 style_flags, v4 rect, char *text)
{
    b32 result = 0;
...
b32
_TsUITextButton(TsUIID id, i32 style_flags, v4 rect, char *text)
{
...
b32
TsUIStyledButton(i32 style_flags, char *text)
{
    v4 rect = {0};
    v4 color = {0};
...
b32
_TsUIToggler(TsUIID id, i32 style_flags, v4 rect, char *text, b32 value)
{
...
b32
TsUIStyledToggler(i32 style_flags, char *text, b32 value)
{
    v4 rect = {0};
    v4 color = {0};
...
f32
_TsUISlider(TsUIID id, i32 style_flags, v4 rect, char *text, f32 value, f32 low, f32 high)
{
    b32 result = 0;
    b32 keyboard_mode = !global_tsui->mouse_mode;
    b32 cursor_over =
...
f32
TsUIStyledSlider(i32 style_flags, char *text, f32 value, f32 low, f32 high)
{
    v4 rect;
    v4 text_color;
...
i32
_TsUIIntSlider(TsUIID id, i32 style_flags, v4 rect, char *text, i32 value, i32 low, i32 high)
{
    b32 result = 0;
    b32 keyboard_mode = !global_tsui->mouse_mode;
    b32 cursor_over =
...
i32
TsUIStyledIntSlider(i32 style_flags, char *text, i32 value, i32 low, i32 high)
{
    v4 rect;
    v4 text_color;
    TsUIUpdateAndGetCurrentState(style_flags, text, &rect, &text_color, v4u(0));
...
void
TsUIStyledTitle(i32 style_flags, char *text)
{
    v4 rect;
    v4 color;
    TsUIUpdateAndGetCurrentState(style_flags, text, &rect, &color, v4u(0));
...
void
TsUIStyledLabel(i32 style_flags, char *text)
{
    v4 rect;
    v4 color;
    TsUIUpdateAndGetCurrentState(style_flags, text, &rect, &color, v4u(0));
...
b32
_TsUICollapsable(TsUIID id, i32 style_flags, v4 rect, char *text)
{
    b32 clicked = _TsUIUpdateClickableWidget(id, style_flags, rect, text);
    HardAssert(global_tsui->widget_count < TSUI_WIDGET_MAX);
    TsUIWidget *widget = TsUIGetNextWidget(id);
...
b32
TsUIStyledCollapsable(i32 style_flags, char *text)
{
    v4 rect = {0};
    v4 color = {0};
    TsUIUpdateAndGetCurrentState(style_flags, text, &rect, &color, v4u(0));
...
void
TsUICollapsableEnd(void)
{
    TsUIPopColumn();
}

void
TsUIStyledDivider(i32 style_flags)
{
    v4 rect;
    v4 color;
...
b32
TsUIStyledWindowCloseButton(i32 style_flags)
{
    v4 rect = {0};
    v4 color = {0};
...
void
TsUIStyledWindowBegin(i32 style_flags, char *title, v4 rect, i32 flags, b32 *open_value_ptr)
{
            ...
            if(!(window->flags & TSUI_WINDOW_FLAG_FIXED_POSITION) && TsUIIDIsNull(global_tsui->hot))
            {
                ...
                else if(!global_tsui->dragging_window)
                {
                    if(V4HasPoint(v4(window->rect.x, window->rect.y,
                                     window->rect.width, 30), v2(global_tsui->cursor_x, global_tsui->cursor_y)))
                    {
...
void
TsUIStyledCanvas(i32 style_flags, char *text,
...
b32
TsUIStyledDropdown(i32 style_flags, char *text)
{
    v4 rect = {0};
    v4 color = {0};
    TsUIUpdateAndGetCurrentState(style_flags, text, &rect, &color, v4u(0));
...
void
TsUICloseCurrentDropdownTree(void)
{
    global_tsui->open_dropdown_stack_size = 0;
}

void
TsUIDropdownEnd(void)
{
    TsUIPopGroupMode();
    TsUIPopPosition();
    TsUIPopSize();
...
v3
_TsUIColorPicker(TsUIID id, i32 style_flags, v4 rect, char *text, v3 color)
{
...
v3
TsUIStyledColorPicker(i32 style_flags, char *text, v3 color)
{
    TsUIDivider();
...
i32
_TsUINotePicker(TsUIID id, i32 style_flags, v4 rect, char *text, i32 note)
{
    b32 keyboard_mode = !global_tsui->mouse_mode;
...
i32
TsUIStyledNotePicker(i32 style_flags, char *text, i32 note)
{
    TsUIDivider();
...
b32
TsUIStyledTileSelect(i32 style_flags, char *text,
                     Ts2dTexture *texture,
                     v4 tilemap_source,
                     i32 *tile_select_x0, i32 *tile_select_y0,
                     i32 *tile_select_x1, i32 *tile_select_y1,
