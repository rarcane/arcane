    ...
    if(ui->input_grid_group_active && !TsUIIDEqual(id, TsUIIDNonInteractable()))
    {
        ...
        if(widget_is_in_active_input_group)
        {
            ...
            switch(ui->current_auto_layout_state.input_grid_active_direction)
            {
                ...
                case TSUI_DIRECTION_RIGHT: { ui->current_auto_layout_state.input_grid_active_position.x += 1; break; }
                case TSUI_DIRECTION_UP:    { ui->current_auto_layout_state.input_grid_active_position.y -= 1; break; }
                case TSUI_DIRECTION_DOWN:  { ui->current_auto_layout_state.input_grid_active_position.y += 1; break; }
                default: break;
            }
...
TsUIBeginFrame(TsUI *ui, TsUIFrameData *frame)
{
    ...
    // NOTE(rjf): Reset auto-layout state.
    {
        ...
        ui->current_auto_layout_state.clip = v4(0, 0, ui->render_width, ui->render_height);
...
void
TsUIPushX(TsUI *ui, f32 x)
{
    HardAssert(ui->x_position_stack_size < TSUI_STACK_MAX);
...
void
TsUIPushY(TsUI *ui, f32 y)
{
    HardAssert(ui->y_position_stack_size < TSUI_STACK_MAX);
...
void
_TsUIPushWidth(TsUI *ui, f32 width, b32 calculate)
{
...
void
_TsUIPushHeight(TsUI *ui, f32 height, b32 calculate)
{
...
void
TsUIPushTextColor(TsUI *ui, v4 color)
{
    HardAssert(ui->text_color_stack_size < TSUI_STACK_MAX);
...
void
TsUIPushTextScale(TsUI *ui, f32 scale)
{
    HardAssert(ui->text_scale_stack_size < TSUI_STACK_MAX);
...
{
    HardAssert(ui->style_flags_stack_size < TSUI_STACK_MAX);
    ui->style_flags_stack[ui->style_flags_stack_size++].flags = ui->current_auto_layout_state.style_flags;
    ui->current_auto_layout_state.style_flags = style_flags;
}

void TsUIPopStyleFlags(TsUI *ui)
{
    if(ui->style_flags_stack_size > 0)
    {
        --ui->style_flags_stack_size;
        ui->current_auto_layout_state.style_flags = ui->style_flags_stack[ui->style_flags_stack_size].flags;
    }
}

void
TsUIPushGroupMode(TsUI *ui, b32 is_column)
{
    HardAssert(ui->group_mode_stack_size < TSUI_STACK_MAX);
...
void
_TsUIPushSize(TsUI *ui, v2 size, b32 calculate_width, b32 calculate_height)
{
    _TsUIPushWidth(ui, size.x, calculate_width);
    _TsUIPushHeight(ui, size.y, calculate_height);
}
...
{
    f32 column_x_upper_bound = ui->current_auto_layout_state.position.x + ui->current_auto_layout_state.size.x;
    f32 column_y_upper_bound = ui->current_auto_layout_state.position.y;
    ...
    if(ui->group_mode_stack_size > 0)
    {
        v4 column_rect = {0};
        {
            column_rect.x = ui->current_auto_layout_state.position.x;
            column_rect.y = ui->current_auto_layout_state.position.y;
...
{
    TsUIPushPosition(ui, position);
    _TsUIPushSize(ui, v2(0, height), 1, 0);
    TsUIPushGroupMode(ui, 0);
}
...
TsUIPopRow(TsUI *ui)
{
    f32 row_x_upper_bound = ui->current_auto_layout_state.position.x;
    f32 row_y_upper_bound = ui->current_auto_layout_state.position.y + ui->current_auto_layout_state.size.y;
    ...
    if(ui->group_mode_stack_size > 0)
    {
        v4 row_rect = {0};
        {
            row_rect.x = ui->current_auto_layout_state.position.x;
            row_rect.y = ui->current_auto_layout_state.position.y;
...
void
TsUIPushClip(TsUI *ui, v4 clip)
{
    HardAssert(ui->clip_stack_size < TSUI_STACK_MAX);
...
TsUIPushInputGridDirection(TsUI *ui, i32 direction)
{
    ...
    switch(direction)
    {
        ...
        {
            ui->current_auto_layout_state.input_grid_active_position.x -= 1;
            break;
        }
        ...
        {
            ui->current_auto_layout_state.input_grid_active_position.y -= 1;
            break;
        }
...
TsUIPopInputGridDirection(TsUI *ui)
{
    if(ui->input_grid_state_stack_size)
    {
        ...
        ui->current_auto_layout_state.input_grid_active_position = ui->input_grid_state_stack[i].input_grid_position;
        ui->current_auto_layout_state.input_grid_active_direction = ui->input_grid_state_stack[i].direction;
    }
}
