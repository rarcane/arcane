...
internal i32 GetOverlappingCollidersAtPosition(ColliderComponent *in_overlapping_colliders[], Shape shape, v2 position, b8 overlap_with)
{
	...
	for (int i = 0; i < 1; i++)
	{
		...
		for (int j = 0; j < chunk->component_set.collider_component_count; j++)
		{
			ColliderComponent *supposed_collider_comp = &chunk->component_set.collider_components[j];
...
internal void AdvanceVelocity()
{
	...
	for (int i = 0; i < chunk_count; i++)
	{
		...
		{
			VelocityComponent *instigator_velocity_comp = &chunk->component_set.velocity_components[j];
			if (instigator_velocity_comp->parent_entity)
			{
				...
				ChunkData *surrounding_chunks[1]; // TODO: Incorporate all surrounding chunks
				surrounding_chunks[0] = GetChunkAtPosition(instigator_position_comp->position);

				for (int k = 0; k < 1; k++)
				{
					ChunkData *against_chunk = surrounding_chunks[k];

					for (int l = 0; l < against_chunk->component_set.collider_component_count; l++)
					{
						Entity *supposed_entity = against_chunk->component_set.collider_components[l].parent_entity;
						if (supposed_entity->entity_id)
						{
							if (&against_chunk->component_set.collider_components[l] != instigator_collider_comp) // Ensure it's not being tested against itself
							{
								PositionComponent *supposed_position_comp = supposed_entity->components[COMPONENT_position];
...
internal void UpdateTriggers()
{
	...
	for (int i = 0; i < chunk_count; i++)
	{
		...
		for (int j = 0; j < chunk->component_set.trigger_component_count; j++)
		{
			TriggerComponent *trigger_comp = &chunk->component_set.trigger_components[j];
			if (trigger_comp->parent_entity)
			{
				...
				ChunkData *surrounding_chunks[1]; // TODO: Incorporate all surrounding chunks
				surrounding_chunks[0] = GetChunkAtPosition(trigger_position_comp->position);
				...
				for (int k = 0; k < 1; k++)
				{
					...
					for (int l = 0; l < against_chunk->component_set.collider_component_count; l++)
					{
						ColliderComponent *supposed_collider_comp = &against_chunk->component_set.collider_components[l];
						if (supposed_collider_comp->parent_entity)
						{
							Entity *supposed_entity = supposed_collider_comp->parent_entity;

							if (&against_chunk->component_set.collider_components[l] != trigger_collider_comp) // Ensure it's not being tested against itself
							{
								PositionComponent *supposed_position_comp = supposed_entity->components[COMPONENT_position];
...
internal void RenderColliders()
{
	...
	for (int i = 0; i < chunk_count; i++)
	{
		...
		for (int j = 0; j < chunk->component_set.collider_component_count; j++)
		{
			ColliderComponent *collider_comp = &chunk->component_set.collider_components[j];
			Entity *entity = collider_comp->parent_entity;
			...
			if (entity)
			{
				...
				for (int k = 0; k < collider_comp->shape.vertex_count; k++)
				{
					int secondPoint = (k == collider_comp->shape.vertex_count - 1 ? 0 : k + 1);
					Ts2dPushLine(
						core->renderer,
						v4(1.0f, 1.0f, 1.0f, 1.0f),
						V2AddV2(view_pos, v2(collider_comp->shape.vertices[k].x * core->camera_zoom, collider_comp->shape.vertices[k].y * core->camera_zoom)),
