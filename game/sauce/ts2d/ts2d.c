...
Ts2dTexture
Ts2dTextureInitFromPNGData(Ts2d *renderer, void *png_data, u32 png_data_size)
{
    ...
    texture = Ts2dTextureInit(renderer, TS2D_TEXTURE_FORMAT_R8G8B8A8, texture_width, texture_height, texture_data);
    stbi_image_free(texture_data);
    return texture;
}

Ts2dTexture
Ts2dTextureLoad(Ts2d *renderer, char *path)
{
    Ts2dTexture texture = {0};
    void *png_data = 0;
    u32 png_data_len = 0;
    ...
    {
        texture = Ts2dTextureInitFromPNGData(renderer, png_data, png_data_len);
...
Ts2dFont
Ts2dFontInitFromPNGAndFNTData(Ts2d *renderer, void *png_data, u32 png_data_size, char *font_data)
{
    Ts2dFont font = {0};
    ...
    font = Ts2dFontInit(renderer, TS2D_TEXTURE_FORMAT_R8G8B8A8, texture_width, texture_height,
...
Ts2dFont
Ts2dFontLoad(Ts2d *renderer, char *png_path, char *fnt_path)
{
    Ts2dFont font = {0};
    ...
    {
        font = Ts2dFontInitFromPNGAndFNTData(renderer, png_data, png_data_len, font_data);
    }
...
Ts2dModel
Ts2dModelInitFromTSMData(Ts2d *renderer, void *data, u32 data_size)
{
    Ts2dModel model = {0};
...
#define TS2D_MODEL_FILE_MATERIAL_REFERENCE_NONE 0
            ...
            if(indices)
            {
                *sub_model = Ts2dSubModelInit(renderer, format, (int)vertex_count, vertices, (int)index_count,
                                              indices, material);
            }
            else
            {
                *sub_model = Ts2dSubModelInitSimple(renderer, format, (int)vertex_count, vertices,
                                                    material);
            }
        }
        
        model = Ts2dModelInit(renderer, sub_model_count, sub_models);
...
Ts2dModel
Ts2dModelLoad(Ts2d *renderer, char *path)
{
    Ts2dModel model = {0};
    void *data = 0;
    u32 data_len = 0;
    ...
    if(data && data_len)
    {
        model = Ts2dModelInitFromTSMData(renderer, data, data_len);
    }
    return model;
}
